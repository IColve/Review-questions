...
-------------------------------------------------------------------------------------------------
                                Fix "Action invoked from another thread"
-------------------------------------------------------------------------------------------------
On Windows, because FileBrowser is calling a FileBrowser.exe file with a Process.Start command,
we have to call that in a thread, to avoid freezing the entire game.
Doing that, when you receive the callback with your result, you might want to do some actions,
but you're not on the Unity main thread, so some actions are not allowed.
In order to fix this, you'll have to use a Dispatcher, that will "save" some code you want to run,
and run it whenever it can on the main thread.

You can find a Dispatcher right here:
https://github.com/nickgravelyn/UnityToolbag/tree/master/Dispatcher
And some explanations on the post from the awesome theknightsofunity blog here:
http://blog.theknightsofunity.com/using-threads-unity/

It's really easy to use.
Instead of directly calling your method like: MyMethod();
You just use: Dispatcher.Invoke(MyMethod); (or InvokeAsync)
You can also write some code instead of calling a method, like so:
Dispatcher.Invoke(() =>
{
    // Here goes your code
});
